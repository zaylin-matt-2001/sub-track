...
- The one allowed composition seam: a Riverpod provider file in
  ...
  `presentation/controllers/` (or `core/di.dart`) that constructs repositories.
...
| `subscriptionNotifierProvider`        | `AsyncNotifierProvider<SubscriptionNotifier, SubscriptionsState>` | Owns the list + derived totals; all mutations.                               |
| `appDatabaseProvider`                 | `Provider<Database>`                                              | The open `sqflite` database.                                                 |
| `subscriptionNotifierProvider`        | `AsyncNotifierProvider<..., SubscriptionsState>`                  | Owns the list + derived totals; depends on settings for multi-currency.      |
...
SubscriptionsState {
  ...
  double monthlyBurnRate;             // M_total
  double yearlyBurnRate;              // A_total
  List<Subscription> subscriptions;   // sorted ascending by nextDueDate
...
- `build()` loads all rows via the repository, runs `CalculateBurnRate` and sort,
  returns the state.
...
- `build()` loads all rows, triggers `AutoAdvanceDueDates`, runs `CalculateBurnRate` (passing exchange rates and base currency), and returns state.
...
│  SubscriptionNotifier   │  depends on Settings + Exchange Rates
└───────────┬─────────────┘
...
5. **Strict dependency rule** — dependencies point inward only.
6. **Immutability** — every entity and model is immutable.
7. **No database calls in UI** — direct DB access inside widgets is forbidden.
8. **Reactive state signals** — updates trigger exactly one state emission that re-computes `M_total` and `A_total` in memory.
...
| Dashboard                                                                                          | `features/subscriptions/presentation/screens/dashboard_screen.dart`           |
...
| Add / Edit modal                                                                                   | `presentation/widgets/add_edit_subscription_sheet.dart`                       |
| Burn-rate engine                                                                                   | `domain/usecases/calculate_burn_rate.dart` (pure)                             |
| Auto-Advance Engine (v1.1)                                                                         | `domain/usecases/auto_advance_due_dates.dart` (pure logic)                    |
| DB migrations v2                                                                                   | `core/database/app_database.dart`                                             |
